...
class EventDetectorTE():
	...
	def entailment(self, premise, hypothesis, premise_upper=True):
		...
		x = self.tokenizer.encode(premise, hypothesis, return_tensors='pt', truncation='only_first', max_length=self.tokenizer.max_len).to('cuda:'+str(self.gpu_devices[0]))
		logits = self.te_model(x)[0]
		
		if self.bert_model_type in ['roberta', 'robertal', 'bartl', 'xlmr_xnli']: # these models have label=2 as the entailment class
			entail_idx = 2
