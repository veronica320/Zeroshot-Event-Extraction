...
output_file = f"{output_dir}/{split}_{setting}_" \
              f"TE:{TE_model}_scft:{srl_consts_for_trg}_tt:{trg_thresh}_tpt:{trg_probe_type}_" \
              f"QA:{QA_model}_scfa:{srl_consts_for_arg}_at:{arg_thresh}_apt:{arg_probe_type}_ih:{identify_head}" \
              f".event.json"
