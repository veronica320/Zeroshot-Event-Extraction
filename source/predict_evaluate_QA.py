...
head_identifier = eval(config.head_identifier)
...
if not evaluate_only:
	if trigger_input_mode == "scratch":
		# input_file = "output_dir/ACE/test/TE/test_m:robertal_t:0.99_a:0.9_['V', 'ARG0', 'ARG1', 'ARG2']_['verb', 'nom']_head:True_tp:topical_pps:None_an:True_cp:whenNone_apt:manual_gdl:pos_neg_srl:celine_new_all.event.json"
		input_file = "output_dir/ERE/E29/TE/E29_m:robertal_t:0.99_a:0.9_['V', 'ARG0', 'ARG1', 'ARG2']_['verb', 'nom']_head:True_tp:topical_pps:None_an:True_cp:whenNone_apt:manual_gdl:pos_neg_srl:celine_new_all.event.json"
	elif trigger_input_mode == "gold_ti":
		# input_file = "output_dir/ACE/test/TE/test_cls_m:robertal_t:0.99_a:0.9_['V', 'ARG0', 'ARG1', 'ARG2']_['verb', 'nom']_head:True_tp:topical_pps:None_an:True_cp:whenNone_apt:manual_gdl:pos_neg_srl:celine_new_all.event.json"
		input_file = "output_dir/ERE/E29/TE/E29_cls_m:robertal_t:0.99_a:0.9_['V', 'ARG0', 'ARG1', 'ARG2']_['verb', 'nom']_head:True_tp:topical_pps:None_an:True_cp:whenNone_apt:manual_gdl:pos_neg_srl:celine_new_all.event.json"
...
if trigger_input_mode == "scratch":
	output_file = f"{output_dir}/{split}_best_trigger_scratch_{'gt_' if gold_trigger else ''}{'cls_' if classification_only else ''}ynm:{YN_QA_model_name}_exm:{EX_QA_model_name}_t:{trg_thresh}_a:{arg_thresh}_{srl_args}_{predicate_type}_head:{head_identifier}_pps:{pair_premise_strategy}_an:{add_neutral}_cp:{const_premise}_apt:{arg_probe_type}_gdl:{tune_on_gdl}.event.json"
elif trigger_input_mode == "gold_ti":
	output_file = f"{output_dir}/{split}_best_trigger_goldti_{'gt_' if gold_trigger else ''}{'cls_' if classification_only else ''}ynm:{YN_QA_model_name}_exm:{EX_QA_model_name}_t:{trg_thresh}_a:{arg_thresh}_{srl_args}_{predicate_type}_head:{head_identifier}_pps:{pair_premise_strategy}_an:{add_neutral}_cp:{const_premise}_apt:{arg_probe_type}_gdl:{tune_on_gdl}.event.json"
elif trigger_input_mode == "gold_titc":
	output_file = f"{output_dir}/{split}_{'gt_' if gold_trigger else ''}{'cls_' if classification_only else ''}" \
	              ...
	              f"{predicate_type}_head:{head_identifier}_pps:{pair_premise_strategy}_an:{add_neutral}_" \
