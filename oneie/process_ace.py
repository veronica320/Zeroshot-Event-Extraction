...
def convert_to_event_only(input_path: str,
                     output_path: str,
                     tokenizer: PreTrainedTokenizer):
    """Converts files to event-only format.
    ...
    with open(input_path, 'r', encoding='utf-8') as r, \
        ...
        for line in r:
            ...
            for sentence in doc['sentences']:
                ...
                entity_span = {e['mention_id']: {'start': e['start'], 'end': e['end']}
                               for e in sentence['entities']}
...
def split_data(input_file: str,
    ...
    with open(input_file, 'r', encoding='utf-8') as r, \
        open(os.path.join(output_dir, 'train.event.json'), 'w') as w_train, \
        open(os.path.join(output_dir, 'dev.event.json'), 'w') as w_dev, \
        open(os.path.join(output_dir, 'test.event.json'), 'w') as w_test:
        for line in r:
...
if __name__ == '__main__':
    ...
    convert_to_event_only(json_path, event_path, tokenizer=tokenizer)
